...
def get_args():
    ...
    parser.add_argument('--conv_recon', help='Comma-separated size:stride:filter:pad of autoencoder convolutional layers', type=str,
        default='3:1:6:0,4:2:16:0,3:1:6:0,4:2:16:1')
...
def train_autoencoder_layer(params, train_dat, ltype='convolutional', sigma=0.1, batch_size=250, epochs=6):
    ...
    if ltype == 'convolutional':
        ...
        del params['pad']
        encoder = lasagne.layers.TransposedConv2DLayer(network, **params)
        network = lasagne.layers.Conv2DLayer(encoder, filter_size=1, stride=1, num_filters=1)
    else:
...
def pretrain_autoencoder(train_data, conv_params, dense_size):
    ...
    if dense_size > 0:
        dense_train, tdata = train_autoencoder_layer({'num_units': dense_size}, tdata, ltype='dense')
        trained_layers.append(dense_train)
